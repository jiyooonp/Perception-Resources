...
from PepperUtils import PepperUtils

class PepperDetection:
    def __init__(self, file_path, yolo_weight_path):
        ...
        self.utils = PepperUtils()
        self.detected_frames = list()
    def __str__(self):
        return self.utils.print_pepperdetection(self)
    def run_detection(self):
        print("Starting detection")
        self.imgs_path = self.utils.get_all_image_path_in_folder(self.path)
    ...
    def predict_pepper(self, img_path, show_result: bool = False, print_result: bool = False):
        ...
        img = self.utils.read_image(img_path)
        ...
        for result in results:
            ...
            for box in boxes:
                one_box = box[0]
                pepper = Pepper(pepper_count)
                xywh = one_box.xywh
                conf = one_box.conf  # Class probabilities for classification outputs
                pepper.set_xywh(xywh[0].cpu().numpy())
                pepper.set_conf(conf)
                detected_frame.add_detected_pepper(pepper)
                pepper_count += 1
        ...
        if print_result:
            self.utils.print_result_boxes(detected_frame)
    ...
    def plot_results(self):
        ...
        for detected_img in self.detected_frames:
            self.utils.draw_img(detected_img)
            ...
            if results[0].probs:
                ...
                for result in results:
                    ...
                    for box in boxes:
                        ...
                        x, y, w, h = box.xywh
                        self.draw_bounding_box(image, box.cls, box.conf, x, y, x + w, y - h)
            else:
...
if __name__ == '__main__':
    PepperDetection = PepperDetection(file_path='/home/jy/PycharmProjects/Perception-Resources/dataset/testbed_video_to_img', yolo_weight_path="../weights/best_22.pt")
